...
public class TraingularKochSnowFlakeTask extends FractalDrawingRenderTask {
	private static double MAX_SIZE=1600.0;
	...
	public TraingularKochSnowFlakeTask(JFXFractalExplorer fractalExplorer, FractalRenderTaskType taskType,Turtle turtle) {
	...
	public void draw() {
		...
		if(preference.getType() == KochSnowFlakeType.TRAINGULAR_KOCH_SNOWFLAKE) {
			_draw(false,true);
		...
		if(preference.getType() == KochSnowFlakeType.TRAINGULAR_ANTI_KOCH_SNOWFLAKE) {
			_draw(true,true);
			paletteColors = ColorPreference.getInstance().getSelectedColorPalette().makeRGBs(numberOfLines, 0);
			rainbowColors = ColorPreference.createRainbowColors(numberOfLines);
			_draw(true,false);
	...
	private void _draw(boolean isAntiSnowFlake,boolean count) {
		double length = MAX_SIZE*preference.getSizePercent()/100.0;
		turtle.penUp();
		if(isAntiSnowFlake) {
			turtle.right(90);
		}else { 
